        ...
        if (newMsgs && newMsgs.length > 0) {
          ...
          if (currentTurn === 5) {
            ...
            if (lastMessage && lastMessage.content === newMsgs[0].content) {
              ...
              let opponentMsg;
              try {
                opponentMsg = await generateRound3OpponentMessage();
                if (!opponentMsg) {
                  throw new Error('Failed to generate opponent message');
                }
              } catch (error) {
                console.error('상대방 메시지 생성 실패:', error);
                opponentMsg = {
                  sender: userStance === "찬성" ? safeRoles.con[0] : safeRoles.pro[0],
                  content: "API 호출 실패",
                  stance: userStance === "찬성" ? "반대" : "찬성",
                  mbti: userStance === "찬성" ? safeRoles.con[0] : safeRoles.pro[0]
                };
              }
  ...
  const getMessages = async () => {
    ...
    if (currentRound === 2) {
      ...
      try {
        ...
        if (!reply || !reply.content) {
          throw new Error('Invalid reply from OpenAI');
        }
        content = removeQuotes(reply.content);
      } catch (error) {
        console.error('OpenAI API 호출 실패:', error);
